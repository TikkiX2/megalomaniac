        ...
        <MainLayout>
            ...
            <div className="mx-auto flex w-full max-w-2xl flex-col gap-8 p-6 lg:p-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
                <div className="flex items-center gap-4">
                    <Link href={finance.purchases.index().url} className="rounded-full p-2 hover:bg-white/5 text-[#92c9a4] transition-all">
                        <span className="material-symbols-outlined">arrow_back</span>
                    </Link>
                    <div>
                        <h2 className="text-3xl font-black tracking-tight text-white">Record Purchase</h2>
                        <p className="text-sm font-medium text-[#92c9a4]">Log your expenses to stay on track.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-6 rounded-2xl bg-[#193322] border border-[#23482f] p-8 shadow-xl">
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        ...
                        <div className="flex flex-col gap-2">
                            <Label htmlFor="purchase_date" className="text-xs font-black uppercase tracking-widest text-[#92c9a4]">Date</Label>
                            <Input
                                id="purchase_date"
                                type="date"
                        ...
                        <div className="flex flex-col gap-2 md:col-span-2">
                            <Label htmlFor="receipt" className="text-xs font-black uppercase tracking-widest text-[#92c9a4]">Receipt (Optional)</Label>
                            <input
                                id="receipt"
                                type="file"
                                onChange={e => setData('receipt', e.target.files ? e.target.files[0] : null)}
                                className="flex w-full rounded-md border border-[#23482f] bg-[#102216] px-3 py-2 text-sm text-[#92c9a4] file:mr-4 file:rounded-full file:border-0 file:bg-primary/10 file:px-4 file:py-1 file:text-xs file:font-black file:text-primary hover:file:bg-primary/20"
                            />
